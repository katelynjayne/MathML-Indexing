...
class LeafNode(Node):
    ...
    def __init__(self, order):
        ...
        #self.freq = [] #Adding this for the fun of it - Megan S. 
...
class BPlusTree(object):
    ...
    def delete(self, key):
        ...
        #node.freq[index].pop() #Added freq - Megan S. 

        if len(node.values[index]) == 0:
            #node.freq.pop(index) #Added freq - Megan S. TODO will this cause problems if node is not leaf?
    ...
    def _borrow_left(node: Node, sibling: Node, parent_index):
        if isinstance(node, LeafNode):  # Leaf Redistribution
            ...
            #freq = sibling.freq.pop(-1) # added freq - Megan S. #TODO am I sure sibling is a leafnode?
            node.keys.insert(0, key)
            node.values.insert(0, data)
            #node.freq.insert(0,freq)
    ...
    def _borrow_right(node: LeafNode, sibling: LeafNode, parent_index):
        if isinstance(node, LeafNode):  # Leaf Redistribution
            ...
            #freq = sibling.freq.pop(0) # added freq - Megan S. #TODO am I sure sibling is a leafnode?
            node.keys.append(key)
            node.values.append(data)
            #node.freq.append(freq)
    ...
    def get_rightmost_leaf(self):
        ...
        while not isinstance(node, LeafNode):
            node = node.values[-1]
            #TODO is this supposed to return something??? - Megan S.
    ...
    def show_all_data(self):
        ...
        while node:
            print(node.keys)
            for i, node_data in enumerate(node.values):
                print('[{}]'.format(', '.join(map(str, node_data))), end=' -> ')
                #if isinstance(node, LeafNode):
                    #print('[{}]'.format(', '.join(map(str, node.freq[i]))), end=' -> ')
